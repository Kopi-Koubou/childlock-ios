...
final class HandBackCopyTests: XCTestCase {
    func testHandBackKeepsChildInstructionPrimaryAndParentEntryVisible() throws {
        ...
        XCTAssertTrue(contents.contains("Nice work. Your app is unlocked now."))
        XCTAssertTrue(contents.contains("HandBackStepRow("))
        XCTAssertTrue(contents.contains("Content unlocked"))
        XCTAssertTrue(contents.contains("The shield is cleared and your content can open again."))
        XCTAssertTrue(contents.contains("HandBackReturnCue("))
        XCTAssertTrue(contents.contains("Back to your app"))
        XCTAssertTrue(contents.contains("Swipe up or press Home once, then tap the app, video, game, or site you were using."))
        ...
        XCTAssertTrue(contents.contains("accessibilityLabel(\"Back to your app. Swipe up or press Home once, then tap the app, video, game, or site you were using.\")"))
    ...
    func testInteractiveControlsDoNotExposeInternalIDsAsSpokenLabels() throws {
        ...
        XCTAssertTrue(handBack.contains("accessibilityLabel(\"Back to your app. Swipe up or press Home"))
    ...
    func testEnforcementSettingsShowOnlyRelevantStartOrStopAction() throws {
        ...
        XCTAssertTrue(dashboard.contains(".disabled(!canStartScreenTimeEnforcement)"))
        XCTAssertTrue(dashboard.contains(".opacity(canStartScreenTimeEnforcement ? 1 : 0.45)"))
    ...
    func testEnforcementSettingsExplainTestFlightHandoffSequence() throws {
        ...
        XCTAssertTrue(dashboard.contains("Same-phone test: lock the parent dashboard or leave Childlock to auto-lock"))
        XCTAssertTrue(dashboard.contains("For child iPad, run these steps on the iPad."))
        XCTAssertTrue(dashboard.contains("For TestFlight: choose the shortest interval, start enforcement, lock the parent dashboard or leave Childlock to auto-lock"))
        XCTAssertTrue(dashboard.contains("Planning labels do not lock content."))
        XCTAssertTrue(dashboard.contains("After selection, start enforcement, lock the parent dashboard or leave Childlock to auto-lock, then hand this device over."))
    ...
    func testOnboardingScreenTimePickerCopyExplainsDoneStep() throws {
        ...
        XCTAssertTrue(onboarding.contains("No Screen Time items selected yet. Use the button below to choose real apps, categories, or websites."))
        XCTAssertTrue(onboarding.contains("Selection saved from Apple's Screen Time picker."))
        XCTAssertTrue(onboarding.contains("Change apps, categories, or websites"))
