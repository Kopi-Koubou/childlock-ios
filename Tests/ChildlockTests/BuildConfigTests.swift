...
final class BuildConfigTests: XCTestCase {
    ...
    func testBuildScriptsAreRepoRelativeAndSafeForUnsignedPreflight() throws {
        ...
        let candidateScript = try readRepoFile("scripts/check-feedback-candidate.sh")
        ...
        let supabaseConfig = try readRepoFile("supabase/config.toml")
        ...
        XCTAssertTrue(validationScript.contains("scripts/check-feedback-candidate.sh --configuration Release"))
        XCTAssertTrue(validationScript.contains("--built-app \"$DERIVED_DATA_PATH/Build/Products/Release-iphoneos/Childlock.app\""))
        XCTAssertTrue(candidateScript.contains("-target Childlock"))
        XCTAssertTrue(candidateScript.contains("-showBuildSettings"))
        XCTAssertTrue(candidateScript.contains("CHILDLOCK_RAPID_TESTING"))
        XCTAssertTrue(candidateScript.contains("ChildlockRapidTestingEnabled"))
        XCTAssertTrue(candidateScript.contains("feedback candidates require NO"))
        XCTAssertFalse(candidateScript.contains("AppSecrets.local.xcconfig"))
        ...
        XCTAssertTrue(validationScript.contains("Brain Break shield shows progress and two answer choices"))
        XCTAssertTrue(validationScript.contains("a wrong answer is replaced by a fresh question"))
        XCTAssertTrue(validationScript.contains("the second correct answer briefly shows Great job"))
        XCTAssertTrue(validationScript.contains("clears the shield with no further child action"))
        XCTAssertTrue(testFlightQAPacketScript.contains("returns\n   automatically to the same unlocked content with no post-answer action"))
        ...
        XCTAssertTrue(production.contains("SUPABASE_SECRET_KEYS"))
        ...
        XCTAssertTrue(supabaseConfig.contains("project_id = \"jkncpveupvozsmbbkvgq\""))
        XCTAssertFalse(supabaseConfig.contains("project_id = \"\""))
        XCTAssertTrue(supabaseConfig.contains("[functions.revenuecat-webhook]"))
        XCTAssertTrue(supabaseReadme.contains("SUPABASE_SECRET_KEYS"))
        ...
        XCTAssertTrue(checklist.contains("scripts/prepare-testflight-qa.sh <build-number>"))
        XCTAssertTrue(testFlightQAPacketScript.contains("scripts/new-hardware-qa-record.sh"))
        XCTAssertTrue(testFlightQAPacketScript.contains("same-phone"))
        XCTAssertTrue(testFlightQAPacketScript.contains("child-ipad"))
        XCTAssertTrue(testFlightQAPacketScript.contains("scripts/launch-readiness-status.sh"))
        XCTAssertTrue(testFlightQAPacketScript.contains("A parent-only iPhone"))
        XCTAssertTrue(testFlightQAPacketScript.contains("denied-notification pass"))
        XCTAssertTrue(testFlightQAPacketScript.contains("second full shield loop"))
        ...
        XCTAssertFalse(backendDeployScript.contains("SUPABASE_URL=\"$SUPABASE_URL\""))
        XCTAssertFalse(backendDeployScript.contains("SUPABASE_SERVICE_ROLE_KEY=\"$SUPABASE_SERVICE_ROLE_KEY\""))
        XCTAssertTrue(backendDeployScript.contains("REVENUECAT_WEBHOOK_SECRET=\"$REVENUECAT_WEBHOOK_SECRET\""))
        XCTAssertTrue(backendDeployScript.contains("Supabase provides SUPABASE_URL and SUPABASE_SECRET_KEYS"))
        ...
        XCTAssertTrue(readinessScript.contains("BLOCKED: do not submit to public App Review yet."))
        XCTAssertTrue(readinessScript.contains("Git tree is dirty"))
        XCTAssertTrue(readinessScript.contains("Same-phone hardware QA record is not launch-complete"))
        XCTAssertTrue(readinessScript.contains("Child-iPad hardware QA record is not launch-complete"))
        XCTAssertTrue(readinessScript.contains("scripts/launch-readiness-status.sh --strict"))
        ...
        XCTAssertTrue(readinessScript.contains("if [[ ! -d \"$root\" ]]"))
        XCTAssertTrue(readinessScript.contains("if [[ ! -d \"$ROOT_DIR/.build/hardware-qa-records\" ]]"))
        ...
        XCTAssertTrue(readinessScript.contains("hardware_record_completion_status"))
        XCTAssertTrue(readinessScript.contains("pending TestFlight build"))
        XCTAssertTrue(readinessScript.contains("incomplete checklist"))
        XCTAssertTrue(readinessScript.contains("required_fields"))
        XCTAssertTrue(readinessScript.contains("describe_hardware_record"))
        XCTAssertTrue(readinessScript.contains("stale commit"))
        XCTAssertTrue(readinessScript.contains("commit unknown; regenerate for current build"))
