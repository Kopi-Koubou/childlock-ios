...
final class ReleaseDocsTests: XCTestCase {
    ...
    func testTestFlightChecklistCapturesLaunchGateEvidence() throws {
        ...
        XCTAssertTrue(checklist.contains("| Child saw `Great job!` and used the bottom-edge `Swipe back` cue to return directly to the now-unshielded previous app/site | Pass / Fail |"))
        ...
        XCTAssertTrue(checklist.contains("Child solves the challenge, sees `Great job!` and `Swipe back`"))
        ...
        XCTAssertTrue(template.contains("Child sees `Great job!` and `Swipe back`, then swipes right along the bottom edge to return directly to the now-unshielded previous app/site"))
        ...
        XCTAssertTrue(template.contains("Child solves the challenge, sees `Great job!` and `Swipe back`, then returns directly to the unlocked previous app/site with the bottom-edge gesture"))
