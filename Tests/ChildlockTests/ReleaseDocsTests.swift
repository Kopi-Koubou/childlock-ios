...
final class ReleaseDocsTests: XCTestCase {
    ...
    func testTestFlightChecklistCapturesLaunchGateEvidence() throws {
        ...
        XCTAssertTrue(normalizedRunSheet.contains("Deny Childlock notification permission before Loop 2"))
        XCTAssertTrue(normalizedRunSheet.contains("Wait for another full production threshold"))
        XCTAssertTrue(normalizedRunSheet.contains("A second shield appearance without completing steps 22–24 is not a two-loop pass."))
        ...
        XCTAssertTrue(checklist.contains("| Same content returned automatically with no post-answer child action | Pass / Fail |"))
        XCTAssertTrue(checklist.contains("| Parent dashboard remained PIN-gated after child use | Pass / Fail |"))
        ...
        XCTAssertTrue(checklist.contains("Child completes both questions directly on the shield, sees `Great job!`"))
        ...
        XCTAssertTrue(checklist.contains("celebration, more-time request"))
        ...
        XCTAssertTrue(template.contains("Loop 2 completes both fresh questions and briefly shows button-free `Great job!`"))
        XCTAssertTrue(template.contains("Loop 2 clears without another touch or opening Childlock"))
        XCTAssertTrue(template.contains("Loop 2 same content is visible in place after automatic shield removal"))
        XCTAssertTrue(template.contains("Loop 1 same content is visible in place after automatic shield removal"))
        ...
        XCTAssertTrue(template.contains("Child completes both shield questions, sees `Great job!`, then returns automatically to the same content with no post-answer action"))
        XCTAssertTrue(template.contains("Shield brain break still works when notifications are denied"))
