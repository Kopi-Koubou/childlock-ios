...
final class ReleaseDocsTests: XCTestCase {
    ...
    func testTestFlightChecklistCapturesLaunchGateEvidence() throws {
        ...
        XCTAssertTrue(normalizedRunSheet.contains("Run one additional full interval"))
        ...
        XCTAssertTrue(checklist.contains("Child answers directly on the shield, sees `Great job!` briefly"))
        ...
        XCTAssertTrue(template.contains("Same content is visible in place after automatic shield removal"))
        ...
        XCTAssertTrue(template.contains("Child answers on the shield, sees `Great job!`, then returns automatically to the same content with no post-answer action"))
