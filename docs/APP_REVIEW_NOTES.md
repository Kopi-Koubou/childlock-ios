...
10. Completing the challenge removes the shield and re-arms monitoring for the
    next interval.
11. After the celebration, the child sees the hand-back screen with `Reopen
...
- Childlock does not automatically return the child to another app after the
  ...
  apps or restore media state, so the hand-back screen says `Reopen your app`
  and tells the child to reopen the now-unshielded app manually.
